...
def create_parser() -> argparse.ArgumentParser:
    ...
    # status subcommand
    subparsers.add_parser("status", help="Show current module-by-module completion status.")

    # sync subcommand
    subparsers.add_parser("sync", help="Synchronize workspace exercises with latest catalog.")

    # check subcommand
    check_parser = subparsers.add_parser("check", help="Validate an exercise solution.")
    check_parser.add_argument("target", nargs="?", help="Path to exercise file or exercise ID.")

    # set subcommand
    set_parser = subparsers.add_parser("set", help="View or update preferences.")
    set_parser.add_argument("--language", choices=["en", "es"], help="Set preferred language (en/es).")

    # view subcommand
    view_parser = subparsers.add_parser("view", help="View exercise problem details.")
    view_parser.add_argument("target", help="Exercise ID or 'next' to view the next unsolved lesson.")
